...
- 🗺️ **Карта сайта (HTML, человеко-читаемая, все ссылки кликабельны)** → [`sitemap.html`](sitemap.html)
- 📐 **Архитектура и карта меню (текст)** → [`SITEMAP.md`](SITEMAP.md)
- 📋 **Реестр страниц с описанием** → [`PAGES.md`](PAGES.md)
- 🤖 **Правила для AI-ассистентов** → [`CLAUDE.md`](CLAUDE.md)
- 📄 **52 HTML-страницы** в корне репо (с человекочитаемыми именами: `about.html`, `blog.html`, `peresborka.html` и т.д.)
- 🎨 **Ассеты Tilda (самохостятся):** `css/`, `js/`, `images/`, `images/cdn/`, `files/`
- 🚫 **`404.html`** — кастомная страница ошибки

## 📌 Важное правило про карту сайта

**При создании любой новой страницы — обязательно обновить `sitemap.html`.**

```bash
python3 .claude/build_sitemap.py
```

Подробности (включая классификацию новой страницы в группу) — в [`CLAUDE.md`](CLAUDE.md).
